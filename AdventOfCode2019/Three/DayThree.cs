...
namespace AdventOfCode2019.Three
{
    public class DayThree : IAdventProblemSet
    {
        ...
        public string PartA()
        {
            int result = CalculateClosestWireIntersection(INPUT);
            return result.ToString();
        }

        public string PartB()
        {
            int result = CalculateClosestWireSteps(INPUT);
            return result.ToString();
        }

        public int CalculateClosestWireSteps(List<string> inputs)
        {
            List<Wire> wires = inputs.Select(i => new Wire(i)).ToList();
            List<int> intersectionSteps = new List<int>();

            Wire wireOne = wires[0];
            Wire wireTwo = wires[1];

            HashSet<Coordinate> wireTwoLocations = wireTwo.GetLocations();
            HashSet<Coordinate> intersections = wireOne.FindIntersections(wireTwoLocations);

            foreach (Coordinate intersection in intersections)
            {
                Coordinate matchingIntersection = wireTwoLocations.First(c => c.Equals(intersection));
                intersectionSteps.Add(intersection.Steps + matchingIntersection.Steps);
            }

            List<int> nonZeroSteps = intersectionSteps.Where(s => s > 0).ToList();
            nonZeroSteps.Sort();
            return nonZeroSteps[0];
        }

        public int CalculateClosestWireIntersection(List<string> inputs)
        {
            List<Wire> wires = inputs.Select(i => new Wire(i)).ToList();
            HashSet<Coordinate> intersectionPoints = new HashSet<Coordinate>();

            for (int i = 0; i < wires.Count; i++)
            {
                Wire wire = wires[i];
                for (int inner = 0; inner < wires.Count; inner++)
                {
                    if (i != inner)
                    {
                        Wire comparingWire = wires[inner];
                        HashSet<Coordinate> intersections = wire.FindIntersections(comparingWire.GetLocations());
                        intersectionPoints.UnionWith(intersections);
                    }
                }
            }

            List<int> distancesFromCentralPort = new List<int>();
            foreach (Coordinate location in intersectionPoints)
            {
                distancesFromCentralPort.Add(CalculateManhattanDistanceToCentralPort(location));
            }

            distancesFromCentralPort.Sort();
            return distancesFromCentralPort.Where(d => d > 0).First();
        }

        private int CalculateManhattanDistanceToCentralPort(Coordinate location)
        {
            int xDistance = Math.Abs(location.X - 0);
            int yDistance = Math.Abs(location.Y - 0);
